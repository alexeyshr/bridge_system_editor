...
const FORMAT_ICONS: Record<string, typeof NewspaperIcon> = {
  article: NewspaperIcon,
  deal_analysis: FlaskConicalIcon,
}
      ...
      <div className="relative w-full max-w-md rounded-xl bg-white p-6 shadow-2xl animate-in zoom-in-95 fade-in duration-200">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-base font-semibold text-[#1f2734]">Archive content</h3>
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          ...
          <button type="button" onClick={onConfirm} disabled={isPending} className="h-9 rounded-lg bg-amber-600 px-3.5 text-sm font-medium text-white transition hover:bg-amber-700 disabled:opacity-60">
...
function RowActionMenu({
  ...
  item: { id: string; title: string; status: string; format: string }
  isGuest: boolean
  isAuthor: boolean
  ...
  return (
    <div className="relative" ref={ref} onBlur={handleBlur}>
      ...
      {open ? (
        <div className="absolute right-0 top-full z-30 mt-1 min-w-[150px] rounded-lg border border-[#e5e7eb] bg-white py-1 shadow-lg animate-in fade-in zoom-in-95 duration-150">
          <Link href={`/dashboard/content/${encodeURIComponent(item.id)}`} className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm text-[#374151] hover:bg-[#f3f4f6]" onClick={close}>
            <SparklesIcon className="size-3.5 text-[#9ca3af]" />
            Open
          </Link>
          {item.format === "deal_analysis" ? (
            <Link href={`/dashboard/content/${encodeURIComponent(item.id)}/study`} className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm text-[#374151] hover:bg-[#f3f4f6]" onClick={close}>
              <FlaskConicalIcon className="size-3.5 text-[#9ca3af]" />
              Studio
            </Link>
          ) : null}
          {!isGuest ? (
            <Link href={`/dashboard/content/${encodeURIComponent(item.id)}/edit`} className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm text-[#374151] hover:bg-[#f3f4f6]" onClick={close}>
...
export default function ContentListPage() {
  ...
  const items = (contentQuery.data?.items ?? []).filter((i: any) => i.format !== "exercise")
  ...
  const unarchiveMutation = trpc.content.unarchive.useMutation({
    onSuccess: () => {
      utils.content.list.invalidate()
      showToast("Content restored to draft", "success")
    },
    onError: (error) => {
  ...
  const totalLabel = items.length === 1 ? "1 item" : `${items.length} items`
  ...
  const formatOptions = useMemo(() => [
    { value: "all" as FormatFilter, label: "All formats" },
    { value: "article" as FormatFilter, label: "Article" },
    { value: "deal_analysis" as FormatFilter, label: "Deal analysis" },
    { value: "auction_lesson" as FormatFilter, label: "Auction lesson" },
    { value: "tournament_recap" as FormatFilter, label: "Tournament recap" },
    { value: "quiz" as FormatFilter, label: "Quiz" },
  ], [])

  const spaceOptions = useMemo(() => [
    { value: "all", label: "All spaces" },
    ...(spacesQuery.data?.spaces ?? []).map((s) => ({ value: s.id, label: s.name })),
  ], [spacesQuery.data])
    ...
    >
      <div className="w-full space-y-5 [[data-sidebar-state=collapsed]_&]:mx-auto [[data-sidebar-state=collapsed]_&]:max-w-5xl">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              <div className="flex size-9 items-center justify-center rounded-lg bg-gradient-to-br from-[#1f2734] to-[#374151]">
                <NewspaperIcon className="size-4.5 text-white" />
              </div>
              <div>
                <h1 className="text-xl font-semibold text-[#1f2734]">Content</h1>
                <p className="text-[13px] text-[#9ca3af]">Drafts, published materials, and archived entries</p>
              </div>
            </div>
          </div>
          {!isGuest ? (
            ...
            >
              <FilePlus2Icon className="size-4 transition-transform group-hover:rotate-12" />
              New draft
            </Link>
          ) : (
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <FilterChip value={formatFilter} onChange={setFormatFilter as (v: FormatFilter | FormatFilter[]) => void} options={formatOptions} allLabel="All formats" allValue="all" />
          <FilterChip value={spaceFilter} onChange={setSpaceFilter as (v: string | string[]) => void} options={spaceOptions} allLabel="All spaces" allValue="all" />
        ...
        {contentQuery.error ? (
          <div className="rounded-lg border border-[#fecdd3] bg-[#fff1f2] px-4 py-3 text-sm text-[#9f1239]">
            Failed to load content: {contentQuery.error.message}
          </div>
        ) : null}
        ...
        {viewMode === "list" ? (
          <div className="border-t border-[#e5e7eb]">
            <div className="hidden border-b border-[#e5e7eb] px-1 py-2 text-[11px] font-semibold uppercase tracking-[0.05em] text-[#9ca3af] md:grid md:grid-cols-[minmax(0,3fr)_100px_130px_90px_100px_44px]">
              <span>Title</span>
              <span>Status</span>
              <span>Format</span>
            ...
            {items.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="group border-b border-[#f0f0f0] px-1 py-3 transition-colors hover:bg-[#fafbfc] md:grid md:grid-cols-[minmax(0,3fr)_100px_130px_90px_100px_44px] md:items-center"
                >
                  <div className="flex items-start gap-2.5 min-w-0">
                    {FormatIcon ? (
                      <FormatIcon className="mt-0.5 size-4 shrink-0 text-[#b0b7c3]" />
                    ) : null}
          ...
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {items.map((item) => {
              ...
              return (
                ...
                >
                  {/* Format accent bar */}
                  <div className={`h-1 rounded-t-xl ${item.format === "deal_analysis" ? "bg-gradient-to-r from-emerald-400 to-emerald-600" : "bg-gradient-to-r from-[#1f2734] to-[#4b5563]"}`} />
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-2">
                      <Link href={`/dashboard/content/${encodeURIComponent(item.id)}`} className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          {FormatIcon ? <FormatIcon className="size-3.5 shrink-0 text-[#b0b7c3]" /> : null}
                    ...
                    <div className="mt-3 flex items-center gap-1 border-t border-[#f3f4f6] pt-3">
                      ...
                      {item.format === "deal_analysis" ? (
                        <Link
                          ...
                          className="inline-flex h-7 items-center gap-1 rounded-md px-2.5 text-xs font-medium text-[#6b7280] transition hover:bg-[#f3f4f6] hover:text-[#374151]"
                        >
        ...
        {items.length === 0 && !contentQuery.isLoading ? (
          <div className="py-20 text-center">
            <div className="mx-auto flex size-16 items-center justify-center rounded-2xl bg-gradient-to-br from-[#f3f4f6] to-[#e5e7eb]">
              <NewspaperIcon className="size-7 text-[#9ca3af]" />
            </div>
            <p className="mt-4 text-base font-medium text-[#374151]">No content found</p>
            <p className="mt-1 text-sm text-[#9ca3af]">Try changing filters or create a new draft.</p>
            {!isGuest ? (
              <Link
                href="/dashboard/content/new"
